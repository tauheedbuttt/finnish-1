...
class WordTypesQuiz {
  ...
  displayQuestionContent(question) {
    ...
    if (
      question.type === "genitive" ||
      question.type === "inessive" ||
      question.type === "adessive" ||
      question.type === "partitive" ||
      question.type === "illative" ||
      question.type === "elative"
    ) {
  ...
  generateVariation(question, existing) {
    ...
    // For i-type words, create wrong forms
    const variations = [];

    // Wrong: keeping -i
    if (word.endsWith("i")) {
      const wrongKeepI = word + "n";
      variations.push(wrongKeepI);
