...
class PastTenseQuiz {
  ...
  displayQuestionContent(question) {
    ...
    if (question.type === "conjugation") {
      questionType.textContent = "Conjugation";
      verbDisplay.textContent = question.verb;
      pronounDisplay.textContent = `(${question.pronoun})`;
      questionEnglish.textContent = question.english;
    } else if (question.type === "possession") {
      questionType.textContent = "Possession (I had)";
      verbDisplay.textContent = question.pronoun;
      pronounDisplay.textContent = "+ oli";
      questionEnglish.textContent = `${question.english}: ${question.context}`;
  ...
  generateOptions(question) {
    ...
    // Get similar answers from same verb or subtopic
    ...
    while (options.length < 4) {
      const variation = this.generateVariation(correct, options);
  ...
  generateVariation(correct, existing) {
    // Create wrong variations for past tense
    const endings = ["n", "t", "i", "tiin", "tte"];
    const base = correct.replace(/[nti]+$/, "");

    for (const ending of this.shuffleArray(endings)) {
      const variation = base + ending;
      if (!existing.includes(variation) && variation !== correct) {
        return variation;
      }
    }
    return null;
