/* ========================================
   Finnish Possessive Sentences Quiz - JavaScript
   Omistusrakenne (Minulla on...)
======================================== */

class PossessiveSentencesQuiz {
  constructor() {
    this.questions = [];
    this.vocabulary = {};
    ...
    // Possessive icons mapping
    this.possessiveIcons = {
      pronouns: "👤",
      names: "👥",
      nouns: "📦",
      "negative-partitive": "❌",
  ...
  async loadQuestions() {
    try {
      ...
      // No subtopics for possessivesentences
      ...
      // No subtopic filter for possessivesentences
  ...
  updateQuestionCount() {
    ...
    if (availableCount) {
      availableCount.textContent = `${filtered.length} questions available`;
    }
    const startBtn = document.getElementById("startBtn");
    startBtn.disabled = filtered.length === 0 ? true : false;
    if (filtered.length === 0 && availableCount) {
      availableCount.classList.add("warning");
    } else if (availableCount) {
      availableCount.classList.remove("warning");
    }
  }

  getFilteredQuestions() {
    // Always return all questions for possessivesentences
  ...
  displayQuestion() {
    ...
    // Display visual based on question
    this.displayPossessiveVisual(question);
  ...
  displayPossessiveVisual(question) {
    const visualComponent = document.getElementById("possessiveComponent");
    const visualIcon = document.getElementById("possessiveIcon");
    const visualWord = document.getElementById("possessiveWord");
    const visualCategory = document.getElementById("possessiveCategory");

    // Reset classes
    visualComponent.className = "possessive-component";

    // Get icon based on subtopic
    const icon = this.possessiveIcons[question.subtopic] || "🎒";
    ...
    const categoryLabels = {
      ...
      names: "Nimet",
      nouns: "Substantiivit",
      "negative-partitive": "Kieltolause",
      feelings: "Tunteet",
    };
    visualCategory.textContent = categoryLabels[question.subtopic] || "Omistus";
    visualComponent.classList.add(question.subtopic);
  }
