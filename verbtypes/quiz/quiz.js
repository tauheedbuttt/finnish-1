/* ========================================
   Finnish Verb Types Quiz - JavaScript
======================================== */
...
class VerbTypesQuiz {
  constructor() {
    ...
    this.verbLabels = {
      type1: "Type 1 (-a/-ä)",
      type2: "Type 2 (-da/-dä)",
      type3: "Type 3 (-la/-na/-ra/-sta)",
      type4: "Type 4 (-ata/-ätä/-uta/-ota)",
      type5: "Type 5 (-ita/-itä)",
      type6: "Type 6 (-eta/-etä)",
    };
  ...
  displayQuestion() {
    ...
    // Display verb visual based on question
    this.displayVerbVisual(question);
  ...
  displayVerbVisual(question) {
    const verbComponent = document.getElementById("verbComponent");
    const verbIcon = document.getElementById("verbIcon");
    const verbWord = document.getElementById("verbWord");
    const verbCategory = document.getElementById("verbCategory");

    // Reset classes
    verbComponent.className = "verb-component";

    // Extract verb from question (in parentheses)
    const verbMatch = question.question.match(/\(([^)]+)\)/);
    const verb = verbMatch ? verbMatch[1] : "";

    // Get verb type from subtopic
    const subtopic = question.subtopic;
    const icon = this.verbIcons[subtopic] || "🔄";
    const label = this.verbLabels[subtopic] || "Verb";

    verbIcon.textContent = icon;
    verbWord.textContent = verb;
    verbCategory.textContent = label;

    // Add type-specific class
    verbComponent.classList.add(subtopic);
  }
